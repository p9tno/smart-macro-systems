...
class Smart_Custom_Fields_Field_Range extends Smart_Custom_Fields_Field_Base
{
	/**
	...
	public function get_field($index, $value)
	{
		...
		$field_id = 'scf-range-' . $index;
		$html = sprintf(
			'<div class="scf-range-wrapper" id="%s"><input type="range" name="%s" value="%s" class="widefat" %s min="%s" max="%s" step="%s" style="max-width: 500px" /> <span class="scf-range-value">%s</span></div>',
			esc_attr($field_id),
			esc_attr($name),
			esc_attr($value),
		...
		jQuery(document).ready(function($) {
			...
			var result = wrapper.find('.scf-range-value');

			range.on('input', function(e) {
				// e.preventDefault();
				console.log(e);
				result.html(range.val());
			});
		});
		</script>
		";
...
function scf_add_range_field()
{
	new Smart_Custom_Fields_Field_Range();
}

add_action('init', 'scf_add_range_field');
